            ...
            layui.use(['layer','table','form'],function () {
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'select'){
                        ...
                        parent.layui.$.each(color,function (index,element) {
                            console.log($(element).val());
                            if(data.color == $(element).val()) {
                                $(element).prop("checked", true);
                            }
                        });
                        $(size).attr('checked',false);
                        parent.layui.$.each(size,function (index,element) {
                            console.log($(element).val());
                            if(data.size == $(element).val()) {
                                $(element).prop("checked", true);
                            }
                        });
                        $(style).attr('checked',false);
                        parent.layui.$.each(style,function (index,element) {
                            console.log($(element).val());
                            if(data.style == $(element).val()) {
                                $(element).prop("checked", true);
                            }
                        });
